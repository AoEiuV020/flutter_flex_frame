...
> 可以，就按现在的规则检查一下项目，调整以符合规则， 
然后按新规则给这个演示项目加上数据库功能， 
刷新数据时读取假数据保存，假装是接口获取，
ui展示时从数据库读取， 真实读取， 

> mobx生成代码出现了InvalidType， 看看配置和代码看有什么问题， 

> 应该是顺序问题， mobx gen用到了 drift gen生成的类，
会不会是mobx gen时drift gen的类还没生成导致出现InvalidType，这种问题有没有什么办法，

> 别瞎改了， build.yaml中不必要的都删掉，然后看看这个链接，学习一下，看能不能设置好顺序， 顺序无关的就别乱试了，， 
@https://github.com/dart-lang/build/issues/2701 

> 我把数据库生成的文件改名了假装不是自动生成的，然后mobx就正常生成了， 
总之你先别管这个问题了， 
继续调整其他代码， 现在还好多报错， 一点一点处理，慢慢来不急，

> 尽量不要使用StatefulWidget，项目里已经有go router + get it + mobx，应该可以实现initState和dispose的替代方案， 设计一下写进rules文件，然后检查所有页面和控件，继续修复代码， 

> 我说的rules规则文件不是README，而是.cursorrules，有且只有这一个文件，把这个文件名也写进规则里，README是给人看的，.cursorrules是给AI看的， 

> 我说的rules规则文件不是README，而是.cursorrules，有且只有这一个文件，把这个文件名也写进规则里，README是给人看的，.cursorrules是给AI看的， 
然后就是重构不要删除已有的功能， 把这点写进rules，
还有很重要的是， 不要车轱辘来回改， 顾头不顾腚，已经知道是错误的方案改成其他方案就不要再改回来了，别两套错误方案来回改， 实在无法解决就停止修改并明确说明遇到的问题， 把这点也写进rules,

> 你怎么连rules都给我乱删了， 我让你加内容， 你给我删东西？

> 你还敢说没删？语言要求和国际化说明都被你删除了， 

> 啥啊你是完全不记得了吗， 我给你回滚了， 你重新编辑一下， 只加规则， 

> 脑残啊， 我让你加什么你都给忘了？加什么语言啊， 语言是因为前面被你删了我才提的， 现在已经回滚了， 语言没问题， 我让你加的是前面说的，
重构不要删除已有的功能，
不要车轱辘来回改， 顾头不顾腚，已经知道是错误的方案改成其他方案就不要再改回来了，别两套错误方案来回改， 实在无法解决就停止修改并明确说明遇到的问题，

> 好了，然后继续处理错误， routes被你删的太多我回滚了， 

> 还有一堆报错， 到处检查一下， 

> 还是一堆的报错，使用analyze检查修复一下， 

> 你是改不完问题了吗，怎么翻来覆去还是有报错， 到底什么问题改不好， 改不好要说出来，继续analyze

> 越改越乱，还记得我一开始是让你改什么的吗？我只是让你加上数据库，你怎么把我ui全部重写了， 原有的ui功能都没了， 还崩溃了， 
我全部回滚了， home你整个检查适配一下从数据库读取数据， 不要修改无关的内容，包括ui效果， 

> ui还是不对， 我又回滚了几个类， 
重中之重的ResponsiveLayout不要乱改， 这里ui是好不容易调好的，别给我改坏了， 

> 继续啊， 你就改那一个类？报错还有一大堆， 你好好analyze，
另外我之前就说过， widget访问getIt尽量使用getter避免写多次getIt，我之前就让你写进rules中， 你什么时候又给我偷偷删除了， 补上这种具体的要求，为的就是简洁，

> context.push报错了， _AssertionError ('package:go_router/src/router.dart': Failed assertion: line 516 pos 12: 'inherited != null': No GoRouter found in context)
另外，原本的假数据什么时候被你删没了， 现在一点数据都没有， 我一开始就说过了， ui从数据库读取， 数据库数据从假数据伪装的接口调用获取保存，

> 你改的啥啊， 路由应该在lib/core/router/routes.dart，规则里没说吗， 没说加上， 

> 你他妈怎么又乱删代码， 连route你都删，你知道那些route有没有使用吗你就删？

> lib/core/di/mock_data.dart一大堆的报错， 你都看不到了吗？

> 修个屁你啥都没修，报错信息你analyze看了吗？

```
怎么回事啊你， 假数据还是刷新不出来， 包括订阅和文章，
另外点击添加订阅还是报错了， 
StateError (Bad state: GetIt: Object/factory with type FeedEditStore is not registered inside GetIt. 
(Did you accidentally do GetIt sl=GetIt.instance(); instead of GetIt sl=GetIt.instance;
Did you forget to register it?))
```

> 你他妈又把我的ui改的一塌糊涂，你是只知道改ui吗？我让你改什么你还记得吗，数据你是改不了了吗？我把ui回滚了， 你重新来， 

> analyze啊，每次都要我说吗， 你这瞎改一通有一堆的报错啊，你都不看看吗？

> ui又被你改没了， 你他妈真的不改ui就修不了代码吗，

> analyze修一下现有的错误， 主要是AI修改数据层把ui改坏了， 我回滚了ui后导致各种报错， 
注意尽量不要删除已有的代码功能， 
不要修改ui，只处理数据，
...
> 现在ui一片漆黑，就一行巨大丑陋的文字， 完整检查一下， 

> 没改好啊， 让你检查你连analyze都不看了吗？我说过的话你都得记住啊， 
...
> 帮我梳理一下添加订阅相关，介绍一下， 然后给哦加几个假数据，假装已有订阅源，假装能从订阅源下载到文章， 
